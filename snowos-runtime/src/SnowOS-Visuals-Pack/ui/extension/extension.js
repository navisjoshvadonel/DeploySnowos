...
const SNOWOS_STATE_DIR = GLib.build_filenamev([GLib.get_user_state_dir(), 'snowos']);
const UI_STATE_FILE = GLib.build_filenamev([SNOWOS_STATE_DIR, 'ui_state.json']);
const UI_MEMORY_FILE = GLib.build_filenamev([SNOWOS_STATE_DIR, 'ui_memory.json']);
...
export default class SnowOSMotionExtension extends Extension {
    ...
    _logInteraction(event, data) {
        GLib.mkdir_with_parents(SNOWOS_STATE_DIR, 0o755);
        let logPath = GLib.build_filenamev([SNOWOS_STATE_DIR, `motion-${event}.log`]);
